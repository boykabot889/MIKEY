...
m.reply(`🚫 أنت محظور، لا يمكنك استخدام أوامر هذا الروبوت!\n\n${user.bannedReason ? `\n💌 *سبب:* 
${user.bannedReason}` : '💌 *سبب:* غير محدد'}\n\n⚠️ *إذا كان هذا الروبوت حسابًا رسميًا وكان لديك دليل يدعم أن هذه الرسالة خطأ، فيمكنك ذكر قضيتك على:*\n\n🤍 ${asistencia}`)
user.antispam++        
return
}
...
conn.reply(m.chat, `لقد تم بيع منتجاتك *💷 ينيس*`, m, fake)
continue
}
let extra = {
match,
...
if (m.yenes)
conn.reply(m.chat, `استخدمته *${+m.yenes}* 💴`, m, fake)
}
break
}}
} catch (e) {
...
if (db.data.chats[m.chat].reaction && m.text.match(/(بضان|ابويا|صاحبي|شادو|عمك|دارك|شاوما|بضاني|اخويا|بعبص|نيك|بعشقك|بحبك|خرا|فيني|بوت|@201211231685|مايكي)/gi)) {
...
const antideleteMessage = `╭•┈•〘❌ مـــضــاد الــحــذف ❌〙•┈• ◊
│⌯الـــمــســتــخــدم: @${participant.split`@`[0]}
│
│⌯بــــعـــت الـــرســالـــه.. ⏱️
╰•┈•〘❌ مــــضــاد الــحــذف ❌〙•┈• ◊`.trim();
