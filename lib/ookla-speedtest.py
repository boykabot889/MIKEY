...
def shell():
    ...
    if not args.mini:
        printer('🌸 *بدء الاختبار...*', quiet)
        printer('🌸 *خادم البحث...*', quiet)
        try:
            ...
            printer('☁️ *تعذر الحصول على قائمة الخادم.*', error=True)
            raise SpeedtestCLIError(get_exception())
        ...
        if args.server and len(args.server) == 1:
            printer('🌟 *الحصول على المعلومات. من الخادم...*', quiet)
        else:
            printer('🌺 *تم اختيار أفضل خادم...*', quiet)
    ...
    printer('\n📚 *مزود خدمة الإنترنت:* %(isp)s' % speedtest.config['client'],
            quiet)
    printer('🍭 *الخادم:* %(sponsor)s\n🔮 *موقع:* %(name)s [%(d)0.2f km] '
            '\n✨️ *كمون:* %(latency)s ms' % results.server, quiet)
    ...
    if args.download:
        ...
        printer('🚀 *تسريح:* %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
        printer('🌸 *تجاوز اختبار التحميل.*', quiet)

    if args.upload:
        speedtest.upload()
        printer('🌻 *يعلو:* %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)

    else:
        printer('🪴 *تخطي اختبار التحميل.*', quiet)
    ...
    if args.share and not machine_format:
        printer('\n🔎 *مشاركة النتيجة:* %s' % results.share())
